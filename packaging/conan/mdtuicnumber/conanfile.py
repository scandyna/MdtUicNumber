...
class MdtUicNumberConan(ConanFile):
  ...
  def export_sources(self):
    ...
    copy(self, "libs/UicNumber/*", source_root, self.export_sources_folder)

  def generate(self):
    tc = CMakeToolchain(self)
    tc.variables["FROM_CONAN_PROJECT_VERSION"] = self.version
    tc.variables["BUILD_UIC_NUMBER"] = "ON"
  ...
  def package_info(self):
    ...
    self.cpp_info.libs = []
    self.cpp_info.set_property("cmake_file_name", "Mdt0UicNumber")
    self.cpp_info.set_property("cmake_target_name", "Mdt0::UicNumber")
