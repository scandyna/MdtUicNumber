...
class MdtUicNumberQtCoreConan(ConanFile):
  ...
  def requirements(self):
    ...
    self.requires(f"mdtuicnumber/{self.version}@scandyna/testing", transitive_headers=True)
    self.requires("qt/5.15.16", transitive_headers=True)
  ...
  def export_sources(self):
    ...
    copy(self, "libs/UicNumber_Qt/*", source_root, self.export_sources_folder)

  def generate(self):
    tc = CMakeToolchain(self)
    tc.variables["FROM_CONAN_PROJECT_VERSION"] = self.version
    tc.variables["BUILD_UIC_NUMBER"] = "OFF"
    tc.variables["BUILD_UIC_NUMBER_QT_CORE"] = "ON"
    tc.variables["BUILD_UIC_NUMBER_QT_GUI"] = "OFF"
  ...
  def package_info(self):
    ...
    self.cpp_info.libs = []
    self.cpp_info.set_property("cmake_file_name", "Mdt0UicNumber_Qt")
    self.cpp_info.set_property("cmake_target_name", "Mdt0::UicNumber_Qt")
